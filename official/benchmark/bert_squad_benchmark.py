...
class BertSquadMultiWorkerBenchmarkFour(BertSquadBenchmarkBase):
  """BERT SQuAD distributed benchmark tests with multiple workers."""

  def __init__(self, output_dir=TMP_DIR, tpu=None, **kwargs):
    super(BertSquadMultiWorkerBenchmarkFour, self).__init__(
        output_dir=output_dir, tpu=tpu, **kwargs)

  def _setup(self):
    """Sets up the benchmark and SQuAD flags."""
    super(BertSquadMultiWorkerBenchmarkFour, self)._setup()
    FLAGS.train_data_path = SQUAD_TRAIN_DATA_PATH
    FLAGS.predict_file = SQUAD_PREDICT_FILE
    FLAGS.vocab_file = SQUAD_VOCAB_FILE
    FLAGS.input_meta_data_path = SQUAD_FULL_INPUT_META_DATA_PATH
  ...
  def _benchmark_common(self, num_workers, all_reduce_alg):
    ...
    FLAGS.datasets_num_private_threads = 32
    FLAGS.model_dir = self._get_model_dir(
        'benchmark_4_gpu_{}_worker_fp16_{}_tweaked'.format(
            num_workers, all_reduce_alg))
    FLAGS.train_batch_size = 4 * num_gpus * num_workers
    FLAGS.all_reduce_alg = all_reduce_alg
  ...
  def benchmark_4_gpu_1_worker_fp16_ring_tweaked(self):
    """4 GPUs per worker, 1 worker, fp16, ring all-reduce."""
    self._benchmark_common(num_workers=1, all_reduce_alg='ring')

  def benchmark_4_gpu_1_worker_fp16_nccl_tweaked(self):
    """4 GPUs per worker, 1 worker, fp16, nccl all-reduce."""
    self._benchmark_common(num_workers=1, all_reduce_alg='nccl')

  def benchmark_4_gpu_2_workers_fp16_ring_tweaked(self):
    """4 GPUs per worker, 2 workers, fp16, ring all-reduce."""
    self._benchmark_common(num_workers=2, all_reduce_alg='ring')

  def benchmark_4_gpu_2_workers_fp16_nccl_tweaked(self):
    """4 GPUs per worker, 2 workers, fp16, nccl all-reduce."""
    self._benchmark_common(num_workers=2, all_reduce_alg='nccl')

  def benchmark_4_gpu_4_workers_fp16_ring_tweaked(self):
    """4 GPUs per worker, 4 workers, fp16, ring all-reduce."""
    self._benchmark_common(num_workers=4, all_reduce_alg='ring')

  def benchmark_4_gpu_4_workers_fp16_nccl_tweaked(self):
    """4 GPUs per worker, 4 workers, fp16, nccl all-reduce."""
    self._benchmark_common(num_workers=4, all_reduce_alg='nccl')

  def benchmark_4_gpu_8_workers_fp16_ring_tweaked(self):
    """4 GPUs per worker, 8 workers, fp16, ring all-reduce."""
    self._benchmark_common(num_workers=8, all_reduce_alg='ring')

  def benchmark_4_gpu_8_workers_fp16_nccl_tweaked(self):
    """4 GPUs per worker, 8 workers, fp16, nccl all-reduce."""
    self._benchmark_common(num_workers=8, all_reduce_alg='nccl')

class BertSquadMultiWorkerBenchmarkOne(BertSquadBenchmarkBase):
  """BERT SQuAD distributed benchmark tests with multiple workers."""
  ...
  def _benchmark_common(self, num_workers, all_reduce_alg):
    ...
    FLAGS.model_dir = self._get_model_dir(
        'benchmark_1_gpu_{}_worker_fp16_{}_tweaked'.format(
            num_workers, all_reduce_alg))
    FLAGS.train_batch_size = 4 * num_gpus * (num_workers + 1)
    FLAGS.all_reduce_alg = all_reduce_alg
...
class BertSquadMultiWorkerBenchmarkTwo(BertSquadBenchmarkBase):
  ...
  def _setup(self):
    """Sets up the benchmark and SQuAD flags."""
    super(BertSquadMultiWorkerBenchmarkTwo, self)._setup()
    FLAGS.train_data_path = SQUAD_TRAIN_DATA_PATH
    FLAGS.predict_file = SQUAD_PREDICT_FILE
    FLAGS.vocab_file = SQUAD_VOCAB_FILE
    FLAGS.input_meta_data_path = SQUAD_FULL_INPUT_META_DATA_PATH
  ...
  def _run_and_report_benchmark(self, use_ds=True, run_eagerly=False):
    ...
    super(BertSquadMultiWorkerBenchmarkTwo, self)._report_benchmark(
        stats=summary,
        wall_time_sec=wall_time_sec,
        min_accuracy=0,
        max_accuracy=1)
  ...
  def _benchmark_common(self, num_workers, all_reduce_alg):
    ...
    FLAGS.datasets_num_private_threads = 32
    FLAGS.model_dir = self._get_model_dir(
        'benchmark_2_gpu_{}_worker_fp16_{}_tweaked'.format(
            num_workers, all_reduce_alg))
    FLAGS.train_batch_size = 4 * num_gpus * num_workers
    FLAGS.all_reduce_alg = all_reduce_alg
  ...
  def benchmark_2_gpu_1_worker_fp16_ring_tweaked(self):
    """2 GPUs per worker, 1 worker, fp16, ring all-reduce."""
    self._benchmark_common(num_workers=1, all_reduce_alg='ring')

  def benchmark_2_gpu_1_worker_fp16_nccl_tweaked(self):
    """2 GPUs per worker, 1 worker, fp16, nccl all-reduce."""
    self._benchmark_common(num_workers=1, all_reduce_alg='nccl')

  def benchmark_2_gpu_2_workers_fp16_ring_tweaked(self):
    """2 GPUs per worker, 2 workers, fp16, ring all-reduce."""
    self._benchmark_common(num_workers=2, all_reduce_alg='ring')

  def benchmark_2_gpu_2_workers_fp16_nccl_tweaked(self):
    """2 GPUs per worker, 2 workers, fp16, nccl all-reduce."""
    self._benchmark_common(num_workers=2, all_reduce_alg='nccl')

  def benchmark_2_gpu_4_workers_fp16_ring_tweaked(self):
    """2 GPUs per worker, 4 workers, fp16, ring all-reduce."""
    self._benchmark_common(num_workers=4, all_reduce_alg='ring')

  def benchmark_2_gpu_4_workers_fp16_nccl_tweaked(self):
    """2 GPUs per worker, 4 workers, fp16, nccl all-reduce."""
    self._benchmark_common(num_workers=4, all_reduce_alg='nccl')

  def benchmark_2_gpu_8_workers_fp16_ring_tweaked(self):
    """2 GPUs per worker, 8 workers, fp16, ring all-reduce."""
    self._benchmark_common(num_workers=8, all_reduce_alg='ring')
